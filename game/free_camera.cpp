...
void FreeCamera::update(GameObject&, float delta)
{
    ...
    float const speed = 10.0f * delta;
    float const sensitivity = 0.3f * delta;

    auto delta_mouse_x = m_last_mouse_x - Engine::Input::mouse_x();
    auto delta_mouse_y = m_last_mouse_y - Engine::Input::mouse_y();
    m_transform->rotate(glm::vec3(0, 1, 0), static_cast<float>(delta_mouse_x) * sensitivity);
    m_transform->rotate(glm::vec3(1, 0, 0), static_cast<float>(delta_mouse_y) * sensitivity);
    ...
    if (Engine::Input::is_key_down(Input::Key::Space)) {
        m_transform->translate(glm::vec3(0, speed, 0));
    }
