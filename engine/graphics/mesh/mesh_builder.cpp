...
MeshBuilder& MeshBuilder::add_vertex(glm::vec3 position)
{
...
MeshBuilder& MeshBuilder::add_uv01(glm::vec2 uv0, glm::vec2 uv1)
{
    m_uv01.push_back(uv0.x);
    m_uv01.push_back(1.0 - uv0.y);
...
MeshBuilder& MeshBuilder::add_builder(MeshBuilder const& other)
{
...
{
    add_vertex(glm::vec3(-size.x, -size.y, 0.0f)).add_uv0(glm::vec2(0, is_y_flipped ? 1 : 0));
    add_vertex(glm::vec3(size.x, -size.y, 0.0f)).add_uv0(glm::vec2(1, is_y_flipped ? 1 : 0));
    add_vertex(glm::vec3(size.x, size.y, 0.0f)).add_uv0(glm::vec2(1, is_y_flipped ? 0 : 1));
    add_vertex(glm::vec3(-size.x, size.y, 0.0f)).add_uv0(glm::vec2(0, is_y_flipped ? 0 : 1));
    add_indicies({ 0, 1, 2 });
    add_indicies({ 0, 2, 3 });
    return *this;
}
...
{
    // Front
    add_vertex(glm::vec3(-size, -size, -size)).add_cube_texture_coord(glm::vec3(-1, -1, -1));
    add_vertex(glm::vec3(size, -size, -size)).add_cube_texture_coord(glm::vec3(1, -1, -1));
    add_vertex(glm::vec3(size, size, -size)).add_cube_texture_coord(glm::vec3(1, 1, -1));
    add_vertex(glm::vec3(-size, size, -size)).add_cube_texture_coord(glm::vec3(-1, 1, -1));
    add_indicies({ 0, 1, 2 });
